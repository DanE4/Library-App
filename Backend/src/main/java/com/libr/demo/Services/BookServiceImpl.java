...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public void updateBook(Book b,int idi) {
        int id =findById(idi).getId();
        Book newBook=new Book(id,b.getTitle(),b.getReleaseyr(),b.getWriter());
        System.out.println(newBook);
        bookRepository.save(newBook);
    }
    @Override
    public Book addBook(String title, int releaseyr, int writerid, int id) {

        Book book = new Book(title,releaseyr,writerService.findById(writerid));
        ...
        if (findById(id) == null) {
            ...
            try {
                updateBook(book,id);
                logger.info("Book udating: "+book.getTitle());
                System.out.println("Book updated: "+book.getTitle());
            }
            catch (Exception e){
                System.out.println("Error updating book: "+book.getTitle()+", Exception: "+e.getMessage());
                logger.warning("Error updating book: "+book.getTitle()+", Exception: "+e.getMessage());
            }
        }
        return book;
    }
    ...
    public Boolean deleteAllByWriterId(int writerid) {
        try {
            bookRepository.findAll().stream().filter(book -> book.getWriter().getId() == writerid)
